...
def add_user_to_g():
    ...
    # Added the following count calculation here
    if g.user:
        g.following_count = len(list(g.user.following))
    else:
        g.following_count = 0
...
def signup():
    ...
    if form.validate_on_submit():
        try:
            ...
            db.session.rollback()  # This line is important to clean up the failed transaction
...
@app.route('/messages/<int:message_id>/toggle_like', methods=['POST'])
...
def homepage():
    ...
    if g.user:
        ...
        # Pass the following_count variable to the template
